...
namespace niedoida {
    ...
    namespace core {
        ...
        HybridKSFockMatrixGenerator::HybridKSFockMatrixGenerator(
            std::shared_ptr<const System> system,
            std::unique_ptr<JKMatrixGenerator> jk_matrix_generator,
            std::unique_ptr<QuasiOneElectronIntegralEngine> xc_integral_engine,
            ...
            m_fock_exchange_contrib(fock_exchange_contrib),
            m_xc_contrib(xc_contrib)
        {
            assert(m_fock_exchange_contrib >= 0);
            assert(m_fock_exchange_contrib <= 1);
            assert(m_xc_contrib >= 0);
            assert(m_xc_contrib <= 1);
        ...
        void HybridKSFockMatrixGenerator::set_density_matrix(
            const arma::mat& P_alpha, const arma::mat& P_beta, bool incremental)
        {
            ...
            // hybrid exchange-correlation potential
            m_K[SPIN_ALPHA] =
                m_fock_exchange_contrib *
                    m_jk_matrix_generator->exchange_matrix(SPIN_ALPHA) +
                m_xc_contrib * m_xc_integral_engine->integrals(SPIN_ALPHA);

            m_K[SPIN_BETA] =
                m_fock_exchange_contrib *
                    m_jk_matrix_generator->exchange_matrix(SPIN_BETA) +
                m_xc_contrib * m_xc_integral_engine->integrals(SPIN_BETA);
        }

        void HybridKSFockMatrixGenerator::calc_matrix_core_gradient(
        ...
        arma::mat HybridKSFockMatrixGenerator::energy_gradient(
            std::shared_ptr<const System> system,
            const arma::mat& C,
            const arma::vec& eps,
            const std::vector<unsigned>& atoms) const
        {
            ...
            arma::mat trK = arma::zeros<arma::mat>(n, 3);
                            ...
                            for (std::size_t j = bJ; j < bJ + nJ; ++j)
                                for (std::size_t k = bK; k < bK + nK; ++k)
                                    for (std::size_t l = bL; l < bL + nL; ++l) {
                                        ...
                                        trK(a, 0) -= 4 * gx * P(j, l) * P(i, k);
                                        trK(a, 0) -= 4 * gx * P(i, l) * P(j, k);
                                        trK(a, 1) -= 4 * gy * P(j, l) * P(i, k);
                                        trK(a, 1) -= 4 * gy * P(i, l) * P(j, k);
                                        trK(a, 2) -= 4 * gz * P(j, l) * P(i, k);
                                        trK(a, 2) -= 4 * gz * P(i, l) * P(j, k);
                                    }
                        break;
                    }
            ...
            e_gradient += trJ + m_fock_exchange_contrib * trK;
            ...
            g += m_xc_contrib * trans(e_gradient);
